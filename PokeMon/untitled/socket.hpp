...
public:
    int init(){
        ...
        do{
            int ret;
            PORTS++;
            my.sin_family = AF_INET;     //指定IP格式
            my.sin_port = htons(PORTS);   //绑定端口号
            my.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");  //表示任何IP   service.sin_addr.s_addr = inet_addr("127.0.0.1");
            ret = ::bind(clientSocket, (SOCKADDR*)&my, sizeof(my));
            if(!ret){
                break;
            }
    ...
    std::string getdata(std::string uname){
        char buffer[1024] = {'f'};
        for(int i = 0; i < uname.size(); i++){
            buffer[i + 1] = uname[i];
        }
